...
export const apiFetchEpic = (action$, _, { ajax }) =>
  action$
    .ofType(GET_API_RESULTS)
    .mergeMap(action =>
      ajax(`http://lcboapi.com/products?q=${action.payload}&access_key=${LCBO_API_KEY}`),
    ) //TODO: use search string later.
    .map(({ response }) => {
      return sendAPIResults(response.result);
    });

export default (...args) => combineEpics(apiFetchEpic)(...args, { ajax: Observable.ajax });
