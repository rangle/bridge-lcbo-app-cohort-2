...
const WishListProduct = wishListProduct => {
  return (
    <li
      ...
      onClick={() => {
        wishListProduct.wishList.find(item => item.id === wishListProduct.id)
    ...
    >
      ...
      <div className="wl-text-wrapper">
        ...
        <div>
          <span>
            ${(wishListProduct.price_in_cents / 100).toFixed(2)}
          </span>
          <span>
            {wishListProduct.package}
          </span>
        </div>
      </div>
      <div id="close_x">&times;</div>
    </li>
  )
}
